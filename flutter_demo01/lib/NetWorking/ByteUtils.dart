...
class ByteDataWriter {
  ...
  void writeString(String value) {
    ...
    write(value.codeUnits);
  }

  void writeUTFString(String value) {
    var tempL = utf8.encode(value);
    writeUint16(tempL.length);
    write(tempL);
  }
...
class ByteDataReader {
  ...
  bool _init(int required) {
    if (remainingLength < required) {
      // print('Not enough bytes to read.');
      return false;
    }
    _clearQueue();
  ...
  String readString([Endian endian]) {
    int length = readUint16(endian);
    return length == 0 ? "" : String.fromCharCodes(read(length));
  }

  String readUTFString([Endian endian]) {
    int length = readUint16(endian);

    return length == 0 ? "" : utf8.decode(read(length));
  }

  String read255String([Endian endian]) {
  ...
  int readDecryptKey() {
    _init(4);
    final value =
        _data.getInt32(_queueCurrentLength - 4, endian ?? this.endian);
    return value;
  }
